	...
	<div class="container">
		<div class="row">
			<div class="col-md-8 col-md-offset-2 sing-in-master">
				...
				<form class="form-horizontal" method="POST" action="{{ url('/vientos/{{$vientos->id}}/edit') }}" enctype="multipart/form-data">
					{{ csrf_field() }}

					<div class="form-group{{ $errors->has('title') ? ' has-error' : '' }}">
						<label for="name" class="sing-in">:Nombre</label>
						<input id="name" type="text" class="form-control" name="title" value="{{ $vientos->title }}" required autofocus placeholder="Agregar Nombre">
					...
					<div class="form-group{{ $errors->has('description') ? ' has-error' : '' }}">
						...
						<input id="name" type="text" class="form-control" name="name" value="{{ $vientos->description }}" required placeholder="Agregar una Descripcion">
					...
					<div class="form-group{{ $errors->has('price') ? ' has-error' : '' }}">
						...
						<input id="name" type="name" class="form-control" name="name" required placeholder="Precio" value="{{ $vientos->price }}">
				...
				<form class="form-horizontal" method="POST" action="{{ route('/vientos/{{$vientos->id}}/delete') }}" enctype="multipart/form-data">
					{{ csrf_field() }}
